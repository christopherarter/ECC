...
| Reasonix | Adapter-backed | managed REASONIX.md memory; flattened project rules; shared skills; commands; agents | MCP and hooks integration deferred until Reasonix documents their on-disk config paths | `./install.sh --profile minimal --target reasonix` | `node tests/lib/install-targets.test.js`; `node tests/scripts/install-apply.test.js` | Do not write the Reasonix-managed `~/.reasonix/config.json`; it holds the API key. |
